...
import {fetchGame, saveGame, updateGame} from './actions';
...
function mapStateToProps(state, props) {
  const {match} = props;
  if (match.params._id) {
    return {game: state.games.find(item => item._id === match.params._id)}
  }
  return {game:null};
}
export default connect(mapStateToProps, {fetchGame, saveGame, updateGame})(GameFormPage);
